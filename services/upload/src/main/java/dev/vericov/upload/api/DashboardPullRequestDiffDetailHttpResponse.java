...
public record DashboardPullRequestDiffDetailHttpResponse(
        UUID id,
        ...
        List<DashboardPullRequestDiffFileHttpResponse> files) {
    public static DashboardPullRequestDiffDetailHttpResponse from(DashboardPullRequestDiffDetails details) {
        var diff = details.diff();
        return new DashboardPullRequestDiffDetailHttpResponse(
                diff.id(),
                ...
                diff.projectLinePct(),
                details.files().stream().map(DashboardPullRequestDiffFileHttpResponse::from).toList());
    }
}
