...
- [ ] Add failing Java and Python contract tests for positive component matching, normalization, root anchoring, specificity, malformed ranges, negation rejection, absolute paths, and parent traversal.
- [ ] Extract a positive `CoveragePathPattern` abstraction in both languages and make ordered ignore rules reuse the same matcher semantics.
- [ ] Define specificity as literal segments and literal characters before the first unescaped wildcard.
- [ ] Run `mvn -pl libraries/coverage-ignore test`.
- [ ] Run `uv run pytest -q tests/test_coverage_ignore.py tests/test_component_path_pattern.py` from `clis/coverage-upload`.
...
- [ ] Add failing tests for valid nested parents/leaves, defaults, owner and gate inheritance, invalid shapes, duplicate/reserved keys, duplicate patterns, unknown fields, and all resource limits.
- [ ] Add immutable `ComponentDefinition`, `ComponentGates`, and `ConfigSnapshot` values.
- [ ] Reject config files larger than 256 KiB before YAML parsing.
- [ ] Parse optional top-level `components`; preserve declaration, path, owner, and ignore order.
- [ ] Canonicalize with sorted object keys, compact separators, array order preserved, and explicit defaults.
- [ ] Add top-level `components` and `config_sha256` to upload JSON and idempotency material.
- [ ] Keep omitted-components behavior repository-only while continuing to send an ignore-only snapshot.
- [ ] Run `uv run pytest -q tests/test_component_config.py tests/test_config.py tests/test_upload.py tests/test_http.py`.
...
- [ ] Add failing tests that load CLI-produced snapshot fixtures and assert exact canonical JSON and SHA-256 parity.
- [ ] Validate keys, shape, owners, gates, path patterns, duplicates, hierarchy limits, and canonical snapshot size.
- [ ] Flatten immutable metadata for each node: parent key, key path, depth, sibling position, effective owners, and effective gates.
- [ ] Resolve files by highest static specificity and fail with conflicting keys on a highest-specificity tie.
- [ ] Add the module to the reactor and service dependencies.
- [ ] Run `mvn -pl libraries/component-config -am test`.
...
- [ ] Add failing tests for missing, ignore-only, and full snapshots; supplied hash mismatch; invalid snapshots before authentication/storage; and exact JDBC bindings.
- [ ] Convert HTTP component values to the shared immutable model.
- [ ] Rebuild and validate the snapshot before any external side effect.
- [ ] Accept transitional ignore-only requests by creating and hashing the normalized empty-components snapshot.
- [ ] Persist `config_snapshot_json` and `config_sha256`; stop treating `ignore_rules` as a separate authority while retaining compatibility reads.
- [ ] Reload the exact snapshot into `QueuedUpload` and include the hash in upload identity.
- [ ] Run `mvn -pl services/upload -am test`.
...
- [ ] Add failing tests proving ignored files are removed before assignment and ambiguity checks.
- [ ] Load, validate, and hash-check the stored snapshot; map corruption to `NonRetryableAnalysisException`.
- [ ] Preserve repository-only behavior when components are empty.
- [ ] Assign each included file to one leaf or `unassigned`; report ambiguity as non-retryable.
- [ ] Build one rollup for every configured node, including zero-valued nodes, by aggregating leaf metrics upward without sibling leakage.
- [ ] Add an `unassigned_files` warning only when unmatched included files exist.
- [ ] Preserve repository total equals root rollups plus `unassigned` for every metric.
- [ ] Run focused coverage-analysis tests.
...
- [ ] Add failing tests for inherited metrics, per-metric overrides, parent descendant totals, leaf totals, `0/0 = 100%`, and no gates on `unassigned`.
- [ ] Emit one blocking `component_config` evaluation per effective gate with component key/path scope.
- [ ] Combine repository and component evaluations into `failed`, `warning`, `passed`, or `not_evaluated`.
- [ ] Keep analysis/upload completion successful when gate status is failed.
- [ ] Disable legacy database `component_coverage` gates from the active repository gate list.
- [ ] Run focused gate and processor tests.
...
- [ ] Add failing JDBC tests for all new report, file, rollup, finding, and gate bindings.
- [ ] Migrate UUID component columns to nullable string keys and rebuild component rollup rows around `(coverage_report_id, component_key)`.
- [ ] Persist snapshot hash, gate status, warnings, hierarchy metadata, effective gates, counters, and file counts transactionally.
- [ ] Query rollups/evaluations in stable path and sibling order and build an immutable nested tree.
- [ ] Add `config_sha256`, `gate_status`, `warnings`, and `components` to the report response while retaining existing top-level fields.
- [ ] Run upload and coverage-analysis persistence/API tests.
...
- [ ] Add failing public-contract checks for the canonical config example, exclusions-before-components wording, stable keys, hierarchy, gates, and future-upload-only behavior.
- [ ] Document a complete `.vericov.yml` example and report outcome semantics.
- [ ] Add a smoke fixture covering ignored, re-included, assigned, overlapping-specific, and unassigned files.
- [ ] Run `uv run pytest -q tests` from the repository root.
- [ ] Run `uv run pytest -q --cov=vericov_coverage_upload --cov-report=term-missing --cov-fail-under=80` from `clis/coverage-upload`.
- [ ] Run `mvn --batch-mode verify`.

## Task 9: Review, Security, And Delivery

- [ ] Review the complete diff for scope, immutability, input validation, migration safety, SQL parameterization, and accidental secret exposure.
- [ ] Run focused tests after every review fix, then repeat all verification commands from Task 8.
- [ ] Confirm Java modules and CLI remain above the repository's 80% coverage gates.
